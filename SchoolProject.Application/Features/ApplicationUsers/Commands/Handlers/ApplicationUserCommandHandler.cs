...
namespace SchoolProject.Application.Features.ApplicationUsers.Commands.Handlers;
                                             ...
                                             ICommandHandler<ChangeUserPasswordCommand, Response<string>>
{
   ...
   public async Task<Response<string>> Handle(AddApplicationUserCommand request, CancellationToken cancellationToken)
   {
      ...
      if (usersCount == 0)
      {
         assignRoleResult = await _userManager.AddToRoleAsync(userToCreate, "Admin");
      }
      else
      {
         assignRoleResult = await _userManager.AddToRoleAsync(userToCreate, "User");
      }
